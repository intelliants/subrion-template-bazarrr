        ...
        <ul class="{$class}{if $pid} menu_{$pid}{/if}">
            ...
            {foreach $data.$pid as $menu}
                ...
                {if in_array($position, array('left', 'right', 'user1', 'user2', 'top'))}
                    <a class="list-group-item m_{$menu.page_name} {if $menu.active} active{/if}" href="{if $menu.url}{$menu.url}{else}{$smarty.const.IA_SELF}#{/if}"{if $menu.nofollow} rel="nofollow"{/if}{if $menu.new_window} target="_blank"{/if}>{$menu.text}</a>
                {else}
                    <li class="m_{$menu.page_name}
                        ...
                        >
                            {$menu.text}
                            {if (isset($data[$menu.el_id]) || isset($menu_children)) && $menu.level == 0  && $position != 'left'}<i class="material-icons">keyboard_arrow_down</i>{/if}
                        </a>
            ...
            {if isset($dropdown) && $menu.level < 1}
                <li class="dropdown dropdown-more">
                    <a class="dropdown-toggle" data-toggle="dropdown" href="#">{lang key='more'} <i class="material-icons">keyboard_arrow_down</i></a>
